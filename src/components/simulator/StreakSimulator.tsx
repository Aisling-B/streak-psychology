  ...
  return (
    <div className="min-h-screen flex items-center justify-center p-4 bg-[#FFFC00]">
      <div className="w-full max-w-[460px] relative">
        <div className={`phone-frame relative transition-all duration-700 bg-background overflow-visible ${isBurnout ? "grayscale opacity-80" : ""}`}>
          
          {/* Status Bar */}
          <div className="flex items-center justify-between px-6 pt-4 pb-2 bg-card/30">
          ...
          <AnimatePresence mode="wait">
            ...
            {/* DAY START OVERLAY: Fixed positioning to prevent clipping */}
            {state.phase === "dayStart" && (
              <motion.div 
                ...
                animate={{ opacity: 1 }} 
                exit={{ opacity: 0 }}
                className="absolute inset-0 bg-background z-[100] flex flex-col items-center justify-center p-6 text-center"
              >
                <div className="w-full max-w-[300px] space-y-6">
                  ...
                  <div className="bg-secondary/20 p-4 rounded-2xl border border-border">
                    <p className="text-[11px] text-muted-foreground leading-relaxed">
                      ...
                      <br/>
                      <span className="text-foreground font-semibold italic mt-2 block">
                        13-14 year olds open this app an average of 841 times a month.
                      </span>
                    </p>
                  </div>
                  ...
                  <button 
                    onClick={startGame} 
                    className="w-full bg-[#FFFC00] text-black py-4 rounded-xl font-black text-sm uppercase shadow-lg active:scale-95 transition-all"
                  >
                    Continue the Loop
                  </button>
                </div>
            ...
            {(state.phase === "choosing" || state.phase === "notification") && (
              <motion.div key="game" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="px-5 pb-5 space-y-4">
                <div className="flex items-center gap-3 py-3 border-b border-border/50">
                  <div className="w-10 h-10 rounded-full bg-secondary flex items-center justify-center text-lg font-bold">👤</div>
                ...
                <div className="relative">
                  {state.tokens > 0 ? (
                    ...
                    <motion.div 
                      ...
                      className="bg-background/95 backdrop-blur-md flex flex-col items-center justify-center text-center p-6 rounded-2xl border-2 border-destructive/20 z-10 shadow-xl"
                    >
                      <span className="text-3xl mb-2">🔋</span>
                      <h3 className="font-bold text-destructive uppercase tracking-tighter">Energy Depleted</h3>
                      <p className="text-[10px] text-muted-foreground mb-4 italic">"I get up and I'm like oh my gosh, the world is spinning."</p>
                      <button onClick={skipStreak} className="w-full bg-[#FFFC00] text-black py-4 rounded-xl font-black text-xs uppercase shadow-lg active:scale-95 transition-all">
                        Finish Day & Rest
                      </button>
                    </motion.div>
                  )}
